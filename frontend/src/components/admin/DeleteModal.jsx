...
export default function DeleteModal({ isOpen, onClose, itemSelected, onDelete, type }) {
  ...
  if (!isOpen || !itemSelected) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 overflow-y-auto bg-black/40 backdrop-blur-sm">
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-2xl p-5 shadow-2xl my-auto">
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-2xl font-bold">Delete {modalTitles[type]}</h2>
          </div>
        ...
        <div>
          <p className="text-lg">Are you sure you want to delete <span className="font-bold text-red-500">({itemSelected.name})</span>?</p>
        </div>
        ...
        <div className="flex justify-end gap-4 mt-8">
          ...
          >
            ...
            {isSubmitting ? 'Deleting...' : `Delete ${modalTitles[type]}`}
          </button>
        </div>
      </div>
    </div>

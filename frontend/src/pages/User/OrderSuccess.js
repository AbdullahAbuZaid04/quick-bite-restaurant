...
import { orderedItems } from '../../data/mockData';
    ...
    <div className="min-h-screen bg-ui-mainBg pb-20">
      ...
      <main className="max-w-xl mx-auto px-4 md:px-6 py-12 md:py-20 flex flex-col items-center text-center">
        ...
        <p className="text-content-subtitle text-sm">
          Your food is being prepared with love. Order <span className="text-brand-primary font-bold">#12345</span> is on the way.
        </p>
        ...
        <div className="w-full mt-12 bg-ui-white rounded-2xl border border-ui-border overflow-hidden">
          ...
          <div className="flex justify-between px-8 py-8 border-b border-ui-mainBg bg-gray-50/50">
            <div className="text-left">
              ...
              <span className="text-2xl font-black text-brand-primary">$49.50</span>
            </div>
            <div className="text-right">
              <span className="text-[10px] font-bold text-content-paragraph uppercase tracking-[0.2em] block mb-1">Est. Arrival</span>
              <span className="text-xl font-black text-brand-primary">25-35 min</span>
            </div>
          </div>
          ...
          <div className="p-6 space-y-3">
            ...
            {orderedItems.map((item) => (
              <div key={item.id} className="flex items-center justify-between p-3 rounded-2xl">
                ...
                <span className="text-xs font-black text-status-delivered bg-status-delivered/10 px-3 py-1.5 rounded-lg border border-status-delivered/20">
                  {item.status}
                </span>
              </div>
            ))}
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 mt-12">
          <button className="flex items-center justify-center gap-2 bg-ui-white text-content-paragraph border-2 border-content-paragraph px-10 py-4 rounded-2xl font-bold hover:bg-content-paragraph hover:text-white transition-all active:scale-95">
            <Home size={18} />
            Back to Home
          </button>

          <button className="flex items-center justify-center gap-2 bg-brand-primary text-ui-white py-5 rounded-2xl font-bold text-sm hover:bg-brand-hover transition-all transform active:scale-95 group">
            Track My Order
